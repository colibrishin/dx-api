...
namespace Fortress::Abstract
{
	...
	void rigidBody::block_window_frame(const std::weak_ptr<rigidBody>& target)
	{
		...
		if (locked_ptr->get_right().get_x() > EngineHandle::get_handle().lock()->get_window_width() ||
			locked_ptr->get_bottom().get_y()> EngineHandle::get_handle().lock()->get_actual_max_y())
		{
			// OOB
		...
		else if (locked_ptr->get_right().get_x() - EngineHandle::get_handle().lock()->get_window_width() > Math::epsilon)
		{
			locked_ptr->m_velocity = locked_ptr->m_velocity.reflect_x();
			locked_ptr->m_position -= {1.0f, 0.0f};
		}
		...
		else if (locked_ptr->get_bottom().get_y() - EngineHandle::get_handle().lock()->get_actual_max_y() > Math::epsilon)
		{
			locked_ptr->m_velocity = locked_ptr->m_velocity.reflect_y();
			locked_ptr->m_position -= {0.0f, 1.0f};
		}
	...
	void rigidBody::apply_gravity()
	{
		...
		const Math::Vector2 diff = get_bottom() - Math::Vector2{
			0.0f, static_cast<float>(EngineHandle::get_handle().lock()->get_actual_max_y())
		};
